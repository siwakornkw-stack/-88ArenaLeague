...
export const alt = "หอเกียรติยศ";
export const size = { width: 1200, height: 630 };
export const contentType = "image/png";

export default async function Image() {
  const [count, kanit] = await Promise.all([
    prisma.league.count({ where: { status: "FINISHED" } }),
  ...
  return new ImageResponse(
    (
      ...
      >
        ...
        <div style={{ fontSize: 72, marginTop: 16, display: "flex" }}>หอเกียรติยศ</div>
        <div style={{ fontSize: 34, color: "rgba(255,255,255,.55)", marginTop: 10, display: "flex" }}>
          แชมป์และดาวซัลโวจาก {count} ฤดูกาลที่จบแล้ว
        </div>
